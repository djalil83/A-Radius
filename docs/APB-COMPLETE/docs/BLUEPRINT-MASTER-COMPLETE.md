...
AI Development Center is the controlled development cockpit for APB.

Developer → AI Plan → Impact → Diff/Preview → Sandbox Test
→ Developer Approval → Execution → Test → Final Approval → Deploy.

AI may use internet research to improve technical knowledge, but external
knowledge never has direct execution authority.

Default:
SUGGEST → PREVIEW → TEST → APPROVAL → EXECUTE.

Production changes, database migrations, network configuration, credentials,
firewall/routing, destructive actions and deployment are approval-gated.
...
Security:
AI receives secret references, never plaintext credentials.
Every proposal and execution has an audit record and rollback plan.
